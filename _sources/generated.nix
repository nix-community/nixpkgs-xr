# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  envision-unwrapped = {
    pname = "envision-unwrapped";
    version = "139f72e2941c4e91091acef6c86712fec566425b";
    src = fetchgit {
      url = "https://gitlab.com/gabmus/envision.git";
      rev = "139f72e2941c4e91091acef6c86712fec566425b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-UPwwT3aNarIABu34fv8Lp8u5Ls1zfEyDnxSnsth+47g=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./envision-unwrapped-139f72e2941c4e91091acef6c86712fec566425b/Cargo.lock;
      outputHashes = {
        
      };
    };
    date = "2025-04-08";
  };
  index_camera_passthrough = {
    pname = "index_camera_passthrough";
    version = "0d3ec30c5cd74e4a3df93d704ecf4a25136afd73";
    src = fetchgit {
      url = "https://github.com/yshui/index_camera_passthrough.git";
      rev = "0d3ec30c5cd74e4a3df93d704ecf4a25136afd73";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-jldpVWnpWEA3bi3lzmG94uCaoZuL+xhaEAtFiZrSGc4=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./index_camera_passthrough-0d3ec30c5cd74e4a3df93d704ecf4a25136afd73/Cargo.lock;
      outputHashes = {
        "vulkano-0.34.0" = "sha256-co2+rDJ1jOee36x9FcLX2Pze9ZItBYHq9/pEq0NhcpY=";
      };
    };
    date = "2024-07-25";
  };
  monado = {
    pname = "monado";
    version = "e926230f346a5b446a9b366cb7cae11224faa5cd";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/monado/monado.git";
      rev = "e926230f346a5b446a9b366cb7cae11224faa5cd";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-fDjhM6zpsViDQ65uBF/Trz/3gzwOwGm42pEewM0Ej4E=";
    };
    date = "2025-04-06";
  };
  opencomposite = {
    pname = "opencomposite";
    version = "aa25936e3884b0da5862280c7ec523e3f3e0c7ae";
    src = fetchgit {
      url = "https://gitlab.com/znixian/OpenOVR.git";
      rev = "aa25936e3884b0da5862280c7ec523e3f3e0c7ae";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Xie1IdFLU1NJcK1rEdW/r7kgass4QyP32DTZ0w6a8DM=";
    };
    date = "2025-04-01";
  };
  wayvr-dashboard = {
    pname = "wayvr-dashboard";
    version = "af6f8b3f169269d42d3d4a15ff33ac2905e0b2bf";
    src = fetchgit {
      url = "https://github.com/olekolek1000/wayvr-dashboard";
      rev = "af6f8b3f169269d42d3d4a15ff33ac2905e0b2bf";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-/irmMmsM/INnOMG1E9ttApfKoSFz68EppcsABrSwM7I=";
    };
    cargoLock."src-tauri/Cargo.lock" = {
      lockFile = ./wayvr-dashboard-af6f8b3f169269d42d3d4a15ff33ac2905e0b2bf/src-tauri/Cargo.lock;
      outputHashes = {
        "keyvalues-parser-0.2.0" = "sha256-LT+WHhan/USzW0EOiuIBPG5j1r9qL4n7Z7ESDxO1xQQ=";
        "wayvr_ipc-0.1.0" = "sha256-ieQaY08Ogl/F3t/p825LBp1lAO3SWH1F8206IPXEgTc=";
        "libmonado-1.3.1" = "sha256-HYYfpYhyo5VmbUdwMTJuAR+2dnMITIGZIPGX9Qsnc/g=";
      };
    };
    date = "2025-04-08";
  };
  wlx-overlay-s = {
    pname = "wlx-overlay-s";
    version = "92c59850b625b8f57fd0073dddaf98f6a1a3c4d1";
    src = fetchgit {
      url = "https://github.com/galister/wlx-overlay-s.git";
      rev = "92c59850b625b8f57fd0073dddaf98f6a1a3c4d1";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-SBh2F38Ywsv8RK7QNZcdmFZASrtRuX0r9haZ4/p5eKE=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./wlx-overlay-s-92c59850b625b8f57fd0073dddaf98f6a1a3c4d1/Cargo.lock;
      outputHashes = {
        "openxr-0.18.0" = "sha256-ktkbhmExstkNJDYM/HYOwAwv3acex7P9SP0KMAOKhQk=";
        "wlx-capture-0.5.0" = "sha256-w1wMKHI1v5EBo/peSXeTyEMLQRjni68BAiNthTzcoCI=";
        "wayvr_ipc-0.1.0" = "sha256-ieQaY08Ogl/F3t/p825LBp1lAO3SWH1F8206IPXEgTc=";
        "libspa-0.8.0" = "sha256-Gub2F/Gwia8DjFqUsM8e4Yr2ff92AwrWrszsws3X3sM=";
        "ovr_overlay-0.0.0" = "sha256-NHzESKsghqI98pkQxEmcNkQ9lTfBsqR9/25msbryi6E=";
        "libmonado-1.3.1" = "sha256-8NA8DLfErX44LIXg5fi2xRDyJaofjpQfILQuu0oaIOQ=";
      };
    };
    date = "2025-04-08";
  };
  xrizer = {
    pname = "xrizer";
    version = "68c00978f5c680aed22972418da531041a5af2aa";
    src = fetchgit {
      url = "https://github.com/Supreeeme/xrizer.git";
      rev = "68c00978f5c680aed22972418da531041a5af2aa";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-EAmX1HwifExtFV9QmqEgvx1mAx5Xm3Lr6VMtLgIkd9c=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./xrizer-68c00978f5c680aed22972418da531041a5af2aa/Cargo.lock;
      outputHashes = {
        "openxr-0.19.0" = "sha256-mljVBbQTq/k7zd/WcE1Sd3gibaJiZ+t7td964clWHd8=";
      };
    };
    date = "2025-04-03";
  };
}
