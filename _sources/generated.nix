# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  index_camera_passthrough = {
    pname = "index_camera_passthrough";
    version = "74edecbf97eaf4ff93032cfecf9b61c179dcffa8";
    src = fetchgit {
      url = "https://github.com/yshui/index_camera_passthrough.git";
      rev = "74edecbf97eaf4ff93032cfecf9b61c179dcffa8";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-mLnRuOxKQuJwgeH0pUj2TNnIug/toJYxTUo4wkJGCxM=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./index_camera_passthrough-74edecbf97eaf4ff93032cfecf9b61c179dcffa8/Cargo.lock;
      outputHashes = {
        "vulkano-0.34.0" = "sha256-co2+rDJ1jOee36x9FcLX2Pze9ZItBYHq9/pEq0NhcpY=";
      };
    };
    date = "2024-06-04";
  };
  monado = {
    pname = "monado";
    version = "28cb225be12a8a55c221f22103c235aff6de3883";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/monado/monado.git";
      rev = "28cb225be12a8a55c221f22103c235aff6de3883";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-lFy0VvaLD4Oyu2TZJnaIWjuaJUZjGGDJS0VsRfIUpcc=";
    };
    date = "2024-06-07";
  };
  opencomposite = {
    pname = "opencomposite";
    version = "49c4b89579fd49c5b40b72924d6593fcd47c5065";
    src = fetchgit {
      url = "https://gitlab.com/znixian/OpenOVR.git";
      rev = "49c4b89579fd49c5b40b72924d6593fcd47c5065";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-Aubf1tupyXzmff3ho/yKx9B3uJ8I0aoZi9zRV3A89Pc=";
    };
    date = "2024-06-01";
  };
  wlx-overlay-s = {
    pname = "wlx-overlay-s";
    version = "b2429bce7f058ca868bc443b30544aa4cd4699a7";
    src = fetchgit {
      url = "https://github.com/galister/wlx-overlay-s.git";
      rev = "b2429bce7f058ca868bc443b30544aa4cd4699a7";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-D0u0iEA0jFBQfx7TA8LeFBjao8zyX6PpjfTrX++atPw=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./wlx-overlay-s-b2429bce7f058ca868bc443b30544aa4cd4699a7/Cargo.lock;
      outputHashes = {
        "vulkano-0.34.0" = "sha256-0ZIxU2oItT35IFnS0YTVNmM775x21gXOvaahg/B9sj8=";
        "ovr_overlay-0.0.0" = "sha256-d38LqhKOp9tHbiK4eU7OPdFvkExqaJN1tB6y2qqPm9U=";
        "wlx-capture-0.3.11" = "sha256-CmFnVfA3MAYXSejn9GpuaNCRu4HiX0CN0j3aN4Pxvjw=";
      };
    };
    date = "2024-06-09";
  };
}
