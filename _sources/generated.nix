# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  envision-unwrapped = {
    pname = "envision-unwrapped";
    version = "d0df943e483f1c768eb35ad60f06a7854e1b3a91";
    src = fetchgit {
      url = "https://gitlab.com/gabmus/envision.git";
      rev = "d0df943e483f1c768eb35ad60f06a7854e1b3a91";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-UnjN7Mzp9D3AgzXnY0ZzoYRWFyl9vgA7nEyKFByskrg=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./envision-unwrapped-d0df943e483f1c768eb35ad60f06a7854e1b3a91/Cargo.lock;
      outputHashes = {
        
      };
    };
    date = "2025-05-02";
  };
  index_camera_passthrough = {
    pname = "index_camera_passthrough";
    version = "0d3ec30c5cd74e4a3df93d704ecf4a25136afd73";
    src = fetchgit {
      url = "https://github.com/yshui/index_camera_passthrough.git";
      rev = "0d3ec30c5cd74e4a3df93d704ecf4a25136afd73";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-jldpVWnpWEA3bi3lzmG94uCaoZuL+xhaEAtFiZrSGc4=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./index_camera_passthrough-0d3ec30c5cd74e4a3df93d704ecf4a25136afd73/Cargo.lock;
      outputHashes = {
        "vulkano-0.34.0" = "sha256-co2+rDJ1jOee36x9FcLX2Pze9ZItBYHq9/pEq0NhcpY=";
      };
    };
    date = "2024-07-25";
  };
  monado = {
    pname = "monado";
    version = "9c14825ec87290d19d374fd0d1b081a67fb2c881";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/monado/monado.git";
      rev = "9c14825ec87290d19d374fd0d1b081a67fb2c881";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-QyDL0q+LmJtg0Rf0CcSkvV35VN9hzWzFi/WV5ooKA50=";
    };
    date = "2025-04-25";
  };
  opencomposite = {
    pname = "opencomposite";
    version = "538639b86c5c71223f11040b09479e5c0a83078c";
    src = fetchgit {
      url = "https://gitlab.com/znixian/OpenOVR.git";
      rev = "538639b86c5c71223f11040b09479e5c0a83078c";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-xVsi06/1bwK4gCMnuPeV/Wwt8SCVMvzsM8TyIbqZiD4=";
    };
    date = "2025-04-27";
  };
  wayvr-dashboard = {
    pname = "wayvr-dashboard";
    version = "872bdf454a4804ca64dfd030ddc0a1afdccd9bc5";
    src = fetchgit {
      url = "https://github.com/olekolek1000/wayvr-dashboard";
      rev = "872bdf454a4804ca64dfd030ddc0a1afdccd9bc5";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-meRjoNQ5jjcLCDV5Nv4lhVVyve1k6G5/iaWg1V6TPYY=";
    };
    cargoLock."src-tauri/Cargo.lock" = {
      lockFile = ./wayvr-dashboard-872bdf454a4804ca64dfd030ddc0a1afdccd9bc5/src-tauri/Cargo.lock;
      outputHashes = {
        "keyvalues-parser-0.2.0" = "sha256-LT+WHhan/USzW0EOiuIBPG5j1r9qL4n7Z7ESDxO1xQQ=";
        "wayvr_ipc-0.1.0" = "sha256-ieQaY08Ogl/F3t/p825LBp1lAO3SWH1F8206IPXEgTc=";
        "libmonado-1.3.1" = "sha256-HYYfpYhyo5VmbUdwMTJuAR+2dnMITIGZIPGX9Qsnc/g=";
      };
    };
    date = "2025-05-04";
  };
  wlx-overlay-s = {
    pname = "wlx-overlay-s";
    version = "f6f330cedd86a25e07615a72a41564b42a0f36cf";
    src = fetchgit {
      url = "https://github.com/galister/wlx-overlay-s.git";
      rev = "f6f330cedd86a25e07615a72a41564b42a0f36cf";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-E0KK8ZvcRfSgS2zUtRiahvEFSqvBN2sTptGQIwdXO0E=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./wlx-overlay-s-f6f330cedd86a25e07615a72a41564b42a0f36cf/Cargo.lock;
      outputHashes = {
        "openxr-0.18.0" = "sha256-ktkbhmExstkNJDYM/HYOwAwv3acex7P9SP0KMAOKhQk=";
        "wlx-capture-0.5.2" = "sha256-rw6konHyvUeb9c5Ln+yUgjJIxymiJfaWKLUL8vDqjnM=";
        "wayvr_ipc-0.1.0" = "sha256-ieQaY08Ogl/F3t/p825LBp1lAO3SWH1F8206IPXEgTc=";
        "libspa-0.8.0" = "sha256-Gub2F/Gwia8DjFqUsM8e4Yr2ff92AwrWrszsws3X3sM=";
        "ovr_overlay-0.0.0" = "sha256-NHzESKsghqI98pkQxEmcNkQ9lTfBsqR9/25msbryi6E=";
      };
    };
    date = "2025-05-02";
  };
  xrizer = {
    pname = "xrizer";
    version = "acbcd1c0ee85ec94025ec3c6b07d372b2d34961c";
    src = fetchgit {
      url = "https://github.com/Supreeeme/xrizer.git";
      rev = "acbcd1c0ee85ec94025ec3c6b07d372b2d34961c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-IT35x+LV+G7cegMUjIrHocp1jSopRe2CYSGLzlbWx4o=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./xrizer-acbcd1c0ee85ec94025ec3c6b07d372b2d34961c/Cargo.lock;
      outputHashes = {
        "openxr-0.19.0" = "sha256-mljVBbQTq/k7zd/WcE1Sd3gibaJiZ+t7td964clWHd8=";
      };
    };
    date = "2025-04-29";
  };
}
